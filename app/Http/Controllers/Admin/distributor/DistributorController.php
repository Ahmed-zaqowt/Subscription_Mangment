    ...
    function getdatasub(Request $request)
    {
        ...
        return DataTables::of($users)
            ...
            ->addColumn('price', function ($qur) {
                $startDate = Carbon::createFromFormat('Y-m-d', $qur->start);
                $endDate = Carbon::createFromFormat('Y-m-d', $qur->end);

                $diffInDays = $endDate->diffInDays($startDate);
                $setting = Setting::orderBy('created_at', 'desc')->first();
                $subscription_price = ($diffInDays * $setting->price) / 30;
                $prcie = number_format($subscription_price, 2);

                return '<div class="badge rounded-pill alert-info"> ' . $prcie  . ' </div>';
            })
